...
import 'package:silah_app/features/consultations/presentation/views/request/widgets/state/requests_loaded.dart';
import 'package:silah_app/features/consultations/presentation/views/request/widgets/state/requests_loading.dart';
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<RequestsBloc, RequestsState>(
      builder: (context, state) {
        if (state is DataRequestLoading || state is DataRequestInitial) {
          return const OperatorsLoading();
        }

        if (state is DataRequestError) {
          return _ScrollableState(
            child: CustomeErrorWidget(
              message: state.message,
              onRetry: () => context.read<RequestsBloc>().add(LoadRequests()),
            ),
          );
        }

        if (state is DataRequestLoaded) {
          if (state.data.isEmpty) {
            return _ScrollableState(
              child: EmptyWidget(
                retryWidget: PrimaryButtonWithProgress(
                  text: Strings.try_again.tr(),
                  onTap: () => context.read<RequestsBloc>().add(LoadRequests()),
                  isLoading: false,
                ),
              ),
            );
